...
def test_task_manager_registers_tasks(task_manager: TaskManager):
    ...
    task_1: TaskId = task_manager.add_task(
        task_type, user, LogArgs(text=["Hello World!"])
    )
    task_2: TaskId = task_manager.add_task(
        task_type, user, LogArgs(text=["Hello World again!"])
    )
